...
def get_coverages(aa_variants):
	'''
	coverage is sometimes spread across multiple lines (e.g. in triallelic
	mutations). This means we have to iterate through the whole file once and
	calculate coverages (in this function) before a second iteration (below)
	assesses if a given line passes both coverage and alternate thresholds.
	'''
	cov_dict={}
	for line_number, line in enumerate(open(aa_variants)):
		...
		if line_number==0:
			...
			gene=line[gene_col]
			AA_pos=line[AA_pos_col]
			sample=line[sample_col]
			gene_pos=gene+'_'+AA_pos
			ref_cnt=int(line[ref_cnt_col])
			alt_cnt=int(line[alt_cnt_col])
			cov_dict.setdefault(sample, {})
			cov_dict[sample].setdefault(gene_pos, ref_cnt)
			cov_dict[sample][gene_pos]+=alt_cnt
	return cov_dict
...
def summarize_prevalences(metadata_dict, coverage_threshold, alternate_threshold, aa_variants):
	...
	header_dict={}
	cov_dict=get_coverages(aa_variants)
	for line_number, line in enumerate(open(aa_variants)):
		line=line.strip().split('\t')
		if line_number==0:
			sample_col=line.index('sample')
			...
			cov_cnt=cov_dict[sample][gene_pos]
			ref_cnt=int(line[ref_cnt_col])
			...
			count_dict[sample].setdefault(gene_mut, {'ref':0, 'alt':0, 'cov':0, 'other':0})
			count_dict[sample][gene_mut]['ref']+=(ref_cnt)
			count_dict[sample][gene_mut]['alt']+=(alt_cnt)
			count_dict[sample][gene_mut]['cov']+=(cov_cnt)
...
def make_counts_tables(count_dict, sorted_mutations, header_dict):
	'''
	...
	for sample in sorted(list(count_dict.keys())):
		for mutation in sorted_mutations:
			pass
			#count_dict[sample][sorted_mutations]
...
#make_counts_tables(count_dict, sorted_mutations, header_dict)